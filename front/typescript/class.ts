import { server, sleep } from "./api";
import { decodeType, record, number, string, array } from "typescript-json-decoder";

type InputGraphType = decodeType<typeof inputGraphDecoder>;
...
(function () {
	...
	const node_event = {
		str2pos: function (data_str: string) {
		...
		draw_edges: function () {
			...
			if (ctx == null) throw `canvas.getContext(2d): ${ctx}`;
	...
	window.addEventListener("load", async function (e) {
		const res = await server.get("class").then(inputGraphDecoder);
		//console.log(res);
		const parent_div = this.document.getElementById("graph_field");
		if (parent_div == null) throw "getElementById graph_field error";
		...
		for (let i = 0; i < res.nodes.length; i++) {
			...
			nodes.id2index.set(id, nodes.elems.length);
		...
		for (let i = 0; i < res.edges.length; i++) {
			...
			const to = nodes.id2index.get(edge.to);
			if (from == null || to == null) {
				throw `internal error, edge ${e} doesn't match`;
			}
			edges.vertex.push([from, to]);
		}
		node_event.draw_edges();

		const save_elem = document.getElementById("nodes_pos_save");
		if (save_elem == null) throw "getElementById nodes_pos_save error";
		save_elem.addEventListener("click", async function (e) {
			const target = e.currentTarget;
			if (target == null) throw "MouseEvent currentTarget is null";

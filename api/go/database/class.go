...
func getClassEdge() ([]ClassEdge, error) {
	var res []ClassEdge
	rows, err := db.Query("SELECT from_id, to_id FROM class_struct_edge")
	if err != nil {
		return res, errors.ErrorWrap(err)
	}
	for rows.Next() {
		var c ClassEdge
		err := rows.Scan(&c.From, &c.To)
		if err != nil {
			return res, errors.ErrorWrap(err)
		}
		res = append(res, c)
...
func (*DatabaseClass) InsertClassEdge(edges []usecase.ClassEdge) error {
	...
	for _, e := range edges {
		query_arg = append(query_arg, e.FromId, e.ToId)
		query_str = append(query_str, "(?,?)")
	}
	_, err := db.Exec("INSERT INTO classroom(from_id, to_id) VALUES "+
		strings.Join(query_str, ","), query_arg...)
	if err != nil {
		return errors.ErrorWrap(err)
	}
